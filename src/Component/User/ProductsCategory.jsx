...
const ProductsCategory = () => {
    ...
    const productFilterFunc = async()=>{  
        ...
        if(subcategory){ 
            const response = await productAllData.filter(value=>{
                ...
                console.log("categoryVal ",categoryVal, value.category);
                console.log(" subcategoryVal", subcategoryVal, value.subCategory);
  ...
  return (
    ...
    {/* {categoryAllData && categoryAllData.filter((item)=>{
        ...
        return( 
            <section key={i}>
                <img src={cv.imageUrl} alt={cv.category || "dooziedo"} className='img-fluid' />  
            </section>
        )
    })} */}
