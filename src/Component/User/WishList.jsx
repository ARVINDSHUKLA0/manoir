...
export default function WishList() { 
    ...
    const addtocart = async(productId, productSize, wishlistId)=>{
        if(productSize.quantity > 0){
        const userdata = localStorage.getItem('userdata')  
        if(userdata){  
            const userdataobj = JSON.parse(userdata) 
            const userId = userdataobj._id   
            await addCartApi(userId, productId, productSize.size).then(()=>removeWishlist(wishlistId)).then(()=>{ 
                dispatch(wishlistdata(userId))
        ...
        <div className="container">
            ...
            <div className="row">  
                { wishlistalldata.map((cv, i)=>{
                    ...
                    const productSize = product.productSize.find(s=>s.size === cv.productSize)  
                    return( 
                    <div className="col-lg-3 col-xl-3  col-md-6 col-6 mb-3" key={i}>
                        ...
                        <div className='text-center d-block product-img-box'>
                            <Link to={`/productview/${product.slug}/${product._id}`}>
                                <img src={`${product.thumbnailImage}`} alt={product.altTag || "doozie do"} className='img-fluid h-100 w-100'/>
                            </Link>
                        ...
                        size : <span>{productSize.size}</span> 
                        </p>
                        <div className='d-flex mb-3 align-items-end justify-content-center gap-3'>
                            ...
                            { product.discount > 0 ? product.salePrice - Math.ceil(product.salePrice * (product.discount/100)) : product.salePrice}
                            </p>  
                            { product.discount > 0?<small className=' fs-12 cl-welcome fs-xs-9'>({product.discount} % off)</small>:null}
                        </div>
                        <div className="text-center">
                        {productSize.quantity > 0 &&
                        <button className='btn btn-blue me-2 btn-s mb-2 text-capitalize' onClick={()=>addtocart(product._id, productSize, cv._id)}><i className="fa-solid fa-cart-shopping fa-lg"></i> move to cart </button>}
                        {!(productSize.quantity > 0) &&
